...
def generate_sudoku():
    ...
    solve_sudoku(board)  # Solve an empty board to generate a valid Sudoku puzzle
    hide_count = 40  # Number of cells to hide for the puzzle (adjust as needed)
...
def solve():
    # Solve the current Sudoku puzzle displayed in the GUI
    board = []
    for i in range(9):
        row = []
        for j in range(9):
...
def generate_and_load():
    # Generate a new Sudoku puzzle and load it into the GUI
...
# Increase window size and center align
window_width = 600
window_height = 600
...
# Create Sudoku grid with sub-grid differentiation and alternate block highlighting
cells = []
for i in range(9):
    row = []
    for j in range(9):
        ...
        # Add thicker borders for sub-grid differentiation
...
solve_button.grid(row=9, column=0, columnspan=3, pady=(10, 20), sticky="ew")  # Center horizontally

generate_button = tk.Button(root, text="Generate Sudoku", command=generate_and_load, **button_style)
generate_button.grid(row=9, column=3, columnspan=3, pady=(10, 20), sticky="ew")  # Center horizontally
